      ...
      <div className="relative z-10 flex min-h-[100vh] items-start justify-center px-5 py-28">
        ...
        >
          <div className="flex shrink-0 items-center justify-between px-8 pt-8 pb-5 sm:px-10 sm:pt-10">
            <h2 className="text-[15px] font-semibold tracking-[-0.03em] text-neutral-900 dark:text-neutral-100">Skills</h2>
            <button
          ...
          <div className="no-scrollbar overflow-y-auto px-8 pb-9 sm:px-10 sm:pb-10">
            <div className="space-y-10">
              {skillGroups.map((group) => (
                <div key={group.title}>
                  <h3 className="text-[15px] font-semibold tracking-[-0.02em] text-neutral-900 dark:text-neutral-100">{group.title}</h3>
                  <p className="mt-1 text-[13px] font-medium leading-[1.5] tracking-[-0.02em] text-neutral-400 dark:text-neutral-400">
                    {group.summary}
                  </p>
                  <div className="mt-3.5 flex flex-wrap gap-1.5">
                    {group.items.map((item) => (
                      <span
                        key={item}
                        className="rounded-full bg-neutral-100 px-2.5 py-1 text-[12px] font-medium tracking-[-0.01em] text-neutral-600 dark:bg-white/[0.08] dark:text-neutral-300"
                      >
                        {item}
                      </span>
                    ))}
