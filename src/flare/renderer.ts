...
// Copyright (c) 2025 Vercel, Inc. Changes: premultiplied canvas, `still` mode for
// reduced motion, rendering pauses offscreen and in hidden tabs.
...
export function createRenderer({
  ...
  const initialize = async () => {
    ...
    if (disposed) return;
    const nextGpu = await init({ label: "k-flare" });
    if (disposed) {
      try {
        nextGpu.dispose();
      } catch {
