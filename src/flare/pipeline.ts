...
// Copyright (c) 2025 Vercel, Inc. Changes: warm flare color, labels.
...
export class FlarePipeline {
  ...
  constructor(private readonly gpu: Gpu, private readonly output: Target) {
    ...
    try {
      this.effects = {
        logo: effect(gpu, fullscreen(logoWgsl), { label: "k-flare-logo" }),
        rim: effect(gpu, fullscreen(rimWgsl), { label: "k-flare-rim" }),
        rimBlurH: effect(gpu, fullscreen(blurWgsl), {
          label: "k-flare-rim-horizontal",
        }),
        rimBlurV: effect(gpu, fullscreen(blurWgsl), {
          label: "k-flare-rim-vertical",
        }),
        composite: effect(gpu, fullscreen(compositeWgsl), {
          label: "k-flare-composite",
        }),
      };
    } catch (error) {
...
export function followLight(current: Point, goal: Point, dt: number): Point {
  const alpha = 1 - Math.exp(-Math.min(Math.max(dt, 0), 0.05) / 0.3);
  return [
    current[0] + (goal[0] - current[0]) * alpha,
    current[1] + (goal[1] - current[1]) * alpha,
  ];
}
